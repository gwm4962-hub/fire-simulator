...
async function runSimulation() {
  ...
  const errors = [];
  if (isNaN(_age) || _age < 20 || _age > 70) errors.push('・開始年齢が異常です（20〜70歳の範囲で設定してください）');
  if (isNaN(_income) || _income <= 0) errors.push('・額面年収を入力してください');
  if (isNaN(_assets) || _assets < 0) errors.push('・現在の資産が異常です');
  if (isNaN(_fire) || _fire <= 0) errors.push('・FIRE達成目標資産を設定してください');
  if (errors.length > 0) {
